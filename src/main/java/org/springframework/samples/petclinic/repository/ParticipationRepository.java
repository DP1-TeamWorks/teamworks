...
public interface ParticipationRepository extends Repository<Participation, Integer>{
	...
	public Collection<Participation> findParticipationByUserAndDeparment(@Param("userId") Integer userId,@Param("projectId") Integer projectId);
	@Query(value="SELECT u FROM Participation u WHERE u.userTW.id = :userId and u.project.id= :projectId and u.finalDate=null" )
    public Participation findCurrentParticipation(@Param("userId") Integer userId,@Param("projectId") Integer projectId);
