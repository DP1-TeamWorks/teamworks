...
public class UserValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
		...
		if(user.getName() == null || user.getName().length()<1||user.getName().length()>25) {
			errors.rejectValue("name", "The name size must be between 1 and 25");
		}
		if(user.getName() == null || !user.getName().matches("^[a-zA-ZÀ-ÿ\\u00f1\\u00d1 ]+$")) {
			errors.rejectValue("name", "The name must contain only letters");
		}
		if(user.getLastname() == null || user.getLastname().length()<1||user.getLastname().length()>120) {
			errors.rejectValue("Lastname", "The Lastname size must be between 1 and 120");
		}
		if(user.getLastname() == null || !user.getLastname().matches("^[a-zA-ZÀ-ÿ\\u00f1\\u00d1 ]+$")) {
			errors.rejectValue("Lastname", "The Lastname must contain only letters");
		}
		if(user.getPassword() == null || user.getPassword().length()<8||user.getPassword().length()>25) {
