...
class DepartmentControllerTest {
	...
	void testInitCreationForm() throws Exception {
		...
		mockMvc.perform(post("/api/departments").session(mockSession).content(Caliidadjson))
	 ...
	 * newArrayList(george, new Owner()));
	 * 
	 * mockMvc.perform(get("/owners")).andExpect(status().isOk()).andExpect(view().
	 * name("owners/ownersList")); } //1
	 * 
	 * @WithMockUser(value = "spring")
	 * 
	 * @Test void testProcessFindUserByLastName() throws Exception {
	 * given(this.UserTWService.findUserByLastName(george.getLastname())).willReturn
	 * (Lists.newArrayList(george));
	 * 
	 * mockMvc.perform(get("/users").param("lastName",
	 * "Franklin")).andExpect(status().is3xxRedirection())
	 * .andExpect(view().name("redirect:/users/" + TEST_USER_ID)); }
	 * 
	 * @WithMockUser(value = "spring")
	 * 
	 * @Test void testProcessFindUserNoOwnersFound() throws Exception {
	 * mockMvc.perform(get("/users").param("lastName",
	 * "Unknown Surname")).andExpect(status().isOk())
	 * .andExpect(model().attributeHasFieldErrors("owner", "lastName"))
	 * .andExpect(model().attributeHasFieldErrorCode("owner", "lastName",
	 * "notFound")) .andExpect(view().name("owners/findUsers")); }
	 * 
	 * @WithMockUser(value = "spring")
	 * 
	 * @Test void testInitUpdateOwnerForm() throws Exception {
	 * mockMvc.perform(get("/owners/{ownerId}/edit",
	 * TEST_USER_ID)).andExpect(status().isOk())
	 ...
	 * mockMvc.perform(post("/owners/{ownerId}/edit", TEST_USERS_ID) .with(csrf())
