...
//	private Tag tags;
	...
	void setup() {
		...
		cyber = new Team();
		cyber.setId(TEST_TEAM_ID);
		cyber.setName("cybergrupo");
		cyber.setIdentifier("cyber");
		cyber.setUsers(users);
		cyber.setDepartments(departments);
	...
	void testInitCreationForm() throws Exception {
		...
		String cyberTestsjson = objectMapper.writeValueAsString(cyber);
		mockMvc.perform(post("/api/team").session(mockSession).content(cyberTestsjson)).andExpect(status().is(200));
	}
	/*
	@WithMockUser(value = "spring")
        @Test
